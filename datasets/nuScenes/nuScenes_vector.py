...
class NuScenesVector(NuScenesTrajectories):
    ...
    def __init__(self, mode: str, data_dir: str, args: Dict, helper: PredictHelper):
        ...
        # Whether to add random flips for data augmentation
            ...
            # self.random_rotates = args['random_rotates']
...
#     @staticmethod
#     def random_rotate(data: Dict):
#         """
#         Helper function to randomly rotate some samples for data augmentation
#         :param data: Dictionary with inputs and ground truth values.
#         :return: data: Dictionary with inputs and ground truth values randomly rotated.
#         """

#         # Rotation matrix
#         phi = (np.random.rand() * 2 * np.pi - np.pi) * 15 / 180
#         rot = np.array([[np.cos(phi), -np.sin(phi)],
#                       [np.sin(phi),  np.cos(phi)]])

#         # Rotate target agent
#         hist = data['inputs']['target_agent_representation']
#         hist[:, :2] = rotate(hist[:, :2], rot)
#         # hist[:, 0] = -hist[:, 0]  # x-coord
#         # hist[:, 4] = -hist[:, 4]  # yaw-rate
#         data['inputs']['target_agent_representation'] = hist

#         # Rotate lane node features
#         lf = data['inputs']['map_representation']['lane_node_feats']
#         lf[:, :, :2] = rotate(lf[:, :, :2], rot)
#         lf[:, :, 2] = (lf[:, :, 2] - phi + np.pi) % (2 * np.pi) - np.pi
#         # lf[:, :, 0] = -lf[:, :, 0]  # x-coord
#         # lf[:, :, 2] = -lf[:, :, 2]  # yaw
#         data['inputs']['map_representation']['lane_node_feats'] = lf

#         # Rotate surrounding agents
#         vehicles = data['inputs']['surrounding_agent_representation']['vehicles']
#         vehicles[:, :, :2] = rotate(vehicles[:, :, :2], rot)
#         # vehicles[:, :, 0] = -vehicles[:, :, 0]  # x-coord
#         # vehicles[:, :, 4] = -vehicles[:, :, 4]  # yaw-rate
#         data['inputs']['surrounding_agent_representation']['vehicles'] = vehicles

#         peds = data['inputs']['surrounding_agent_representation']['pedestrians']
#         peds[:, :, :2] = rotate(peds[:, :, :2], rot)
#         # peds[:, :, 0] = -peds[:, :, 0]  # x-coord
#         # peds[:, :, 4] = -peds[:, :, 4]  # yaw-rate
#         data['inputs']['surrounding_agent_representation']['pedestrians'] = peds

#         # Rotate groud truth trajectory
#         fut = data['ground_truth']['traj']
#         fut[:, :2] = rotate(fut[:, :2], rot)
#         # fut[:, 0] = -fut[:, 0]  # x-coord
#         data['ground_truth']['traj'] = fut

#         return data

# def rotate(p: np.ndarray, R=None, phi=None):
#     """
#         Rotate a set of points with rotation matrix or a given radians.
#         One of R or phi must be given.
#         :param p: a set of points with shape [*, 2].
#         :param R: rotation matrix with shape [2, 2].
#         :param phi: rotation radians.
#         :return: p: rotated points.
#     """
    
#     if R is None:
#         assert phi is not None, 'One of R or phi must be given!'
#         R = np.array([[np.cos(phi), -np.sin(phi)],
#                       [np.sin(phi),  np.cos(phi)]])
#     shape = p.shape
#     p = p.reshape(-1, 2)
    
#     return np.squeeze((R @ (p.T)).T).reshape(*shape)