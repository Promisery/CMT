...
class Residual(nn.Module):
    def __init__(self, dim, scale_residual = False, scale_residual_constant = 1., drop_path = 0.):
        super().__init__()
    ...
    def forward(
        ...
        if exists(attn_mask):
            ...
            if attn_mask.ndim == 2:
                ...
                attn_mask = rearrange(attn_mask, '(b h) i j -> b h i j', h=self.heads)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if deepnorm:
            ...
            scale_residual = True
            scale_residual_constant = (2 * depth) ** 0.25 if scale_residual_constant == 1 else scale_residual_constant
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            residual = residual_fn(dim, scale_residual = scale_residual, scale_residual_constant = scale_residual_constant, drop_path = drop_path)
    ...
    def forward(
        ...
        for ind, (layer_type, (norm, block, residual_fn)) in enumerate(zip(self.layer_types, self.layers)):
            ...
            if layer_type == 'a':
                ...
                out, inter = block(x, context = context, mask = mask, context_mask = context_mask, attn_mask = cross_mask, prev_attn = prev_cross_attn)
            elif layer_type == 'f':
                out = block(x)
