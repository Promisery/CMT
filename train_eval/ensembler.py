...
class Ensembler:
    ...
    """
    def __init__(self, cfgs: List[Dict], data_root: str, data_dir: str, checkpoint_paths: List[str]):
        """
        ...
        for idx, (cfg, checkpoint_path) in enumerate(zip(cfgs, checkpoint_paths)):
            cfg['decoder_args']['k'] = cfg['decoder_args']['num_modes']
            if idx == 0:
                # Initialize dataset
                ...
                self.dl = torch_data.DataLoader(test_set, cfg['batch_size'], shuffle=False,
                                                num_workers=cfg['num_workers'], pin_memory=True,
                                                    drop_last=False, persistent_workers=cfg['num_workers'] > 0,
                                                    worker_init_fn=u.seed_worker, generator=val_g)
                
                # Initialize metrics
                self.metrics = [initialize_metric(cfg['test_metrics'][i], cfg['test_metric_args'][i], test_set.helper)
                        for i in range(len(cfg['test_metrics'])) if cfg['test_metrics'][i] != 'pi_bc']
    ...
    def evaluate(self, output_dir: str):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.dl):
                ...
                for model in self.models:
                    # Forward pass
                    _predictions = model(data['inputs'])
                    traj.append(_predictions['traj'])
                    probs.append(_predictions['probs'] / len(self.models))
                ...
                traj, probs, _ = gmrc(
                    ...
                    log_probs,
                    k=10,
                    n_sample=1024 * len(self.models),
                )
                
                predictions = data['inputs'].copy()
        ...
        with open(os.path.join(output_dir, 'results', "results.txt"), "w") as out_file:
            for metric in self.metrics:
                ...
                output = metric.name + ': ' + format(avg_metric, '0.6f')
                print(output)
